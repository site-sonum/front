            ...
            {array ? (
                array.map((e)=> {
                    switch(e.type){
                        case 'badge':
                            return (
                                <div className={(position[e.position] == 'left' || position[e.position] == 'right') ? `${position[e.position]}` : 'center'}
                                style={{margin:'32px'}}>
                        ...
                        case 'bouton':
                            return(
                                <Button data={e} rows={1}/>
                            )
                        // eslint-disable-next-line no-unreachable
                        break
                        case 'groupe-de-boutons':
                            return(
                                <ButtonGroup data = {e} rows = {1}/>
                            )
                        break
                        case 'carte-verticale':
                            return (
                                <VerticalCard data = {e} rows = {1}/>
                            )
                        break
                        case 'carte-horizontale':
                            return (
                                <HorizontalCard data = {e} rows = {1}/>
                            )
                        // eslint-disable-next-line no-unreachable
                        break
                        case 'flux-de-publications':
                            return(
                                <div key = {e.id}
                                    ...
                                    <div className='grid2'>
                                    {fluxPublications && fluxPublications.map(flux=>{
                                        return (
                                            <HorizontalCard data = {flux} rows = {2} displayText={false}  fluxTwo={true}/>
                                        )
                                    })}
                                </div>
                            </div>
                        ...
                        case 'navigation':
                            return (
                                <AnchorNavigator data = {e}/>
                            )
                        break
                        case 'ancre':
                            return (
                                <div className = 'translate-header-margin' id = {e.ancre}/>
                            )
                        break
                        case 'bloc-de-breves':
                            return (
                                <BlocCards articles={breves} type="breve"/>
                            )
                        break
                        case 'bloc-de-publications-strategiques':
                            return (
                                <BlocCards articles={strategiques} type="rapport-strategique"/>
                            )
                        break
                        case 'bloc-de-rapports-de-recherches':
                            return (
                                <BlocCards articles={recherches} type="etude"/>
                            )
                        break
                        case 'citation':
                            return (
                        ...
                        case 'lien-hypertexte':
                            return(
                                <NavLink data={e} rows = {1}/>
                            )
                        break
                        case 'telechargement':
                            return(
                                <DownloadLink data ={e} rows = {1}/>
                            )
                        break
                        case 'carte-telechargement':
                            return(
                                <DownloadCard  data = {e} rows = {1}/>
                            )
                        break
                        case 'accordeon':                  
                            return(
                                <Accordion data={e} rows = {1}/>
                                )
                        break
                        case 'iframe':
                            return(
                                <Iframe data = {e} rows = {1}/>
                            )
                        break
                        ...
                        case 'flux-dispositifs':
                            return(
                                <ToolsDevicesContainer data={dataToolsDevices} slug = {slug} type={"Dispositif"}/>
                            )
                        break
                        case 'flux-outils':
                            return(
                                <ToolsDevicesContainer data={dataToolsDevices} slug = {slug} type={"Outil"}/>
                            )
                        break
                        case 'section':
                                                ...
                                                {currentSection.elements.map(c=>{
                                                    switch(c.type){
                                                        ...
                                                        case 'bloc-de-texte':
                                                            return (
                                                            <div key = {c.id}>
                                                                <TextArea texte={c.texte} position={null} row={currentSection.rows}/>
                                                            </div>
                                                            )
                                                        // eslint-disable-next-line no-unreachable
                                                        break
                                                        ...
                                                        case 'groupe-de-boutons':
                                                            return(
                                                                <ButtonGroup data = {c} rows = {currentSection.rows}/>
                                                            )
                                                        break
                                                        case 'carte-verticale':
                                                            return (
                                                                <VerticalCard data = {c} rows = {currentSection.rows}/>
                                                            ) 
                                                        // eslint-disable-next-line no-unreachable
                                                        break
                                                        case 'carte-horizontale':
                                                            return (
                                                                <HorizontalCard data = {c} rows = {currentSection.rows}/>
                                                            ) 
                                                            // eslint-disable-next-line no-unreachable
                                                        break
                                                        case 'lien-hypertexte':
                                                            return(
                                                                <NavLink data = {c} rows = {currentSection.rows} />
                                                            )
                                                        break
                                                        case 'telechargement':
                                                            return(
                                                                <DownloadLink data = {c} rows = {currentSection.rows} />
                                                            )
                                                        break
                                                        case 'carte-telechargement':
                                                            return(
                                                                <DownloadCard data = {c} rows = {currentSection.rows}/>
                                                            )
                                                        break
                                                    }
                                                })}
