...
export const HorizontalCard = ({
  ...
  useEffect(() => {
    setType(
      data.type[0].toUpperCase() + data.type.slice(1).split("-").join(" "),
    );
    switch (data.type) {
      case "rapport-stratégique":
        var flowPrefix =
          displayText == false ? "nos-ressources/rapports-strategiques/" : "";
        setSlug(
          flowPrefix +
        ...
        >
          ...
          >
            ...
            <div className="absolute bottom-0 left-4">
              <img
                src={RightArrow}
                style={{ width: "15px", transform: "translateY(-0.37rem)" }}
                alt=""
              />
              <div className="right-arrow-card-transform">
                <span style={{ marginLeft: "0.30rem", fontSize: "12px" }}>
                  {" "}
        ...
        >
          <div className="">
            <Link
              to={data.lien}
              target={
                data.lien && data.lien.includes("https://") ? "_blank" : null
              }
            ...
            >
              {imageUrl && (
                <img
                  className="w-full h-full md:w-48"
                  src={HOST_URL + imageUrl}
                  alt=""
                />
              )}
              <div
                className={`flex flex-col justify-between p-5 leading-normal mt-1 ${!imageUrl && "ml-5"}`}
              >
                ...
                <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-black pt-4">
                  {data.titre}
                </h5>
                ...
                <i
                  ...
                  style={{ color: "#1132DE", fontSize: "25px" }}
                ></i>
              </div>
            </Link>
          </div>
