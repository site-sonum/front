...
export const DownloadCard = ({data, rows}) =>{
    ...
    return (
        ...
        <>
            ...
            <div key={localData.id} style={ rows == 1 ? { display: 'flex', justifyContent: localData.position === 'Centre' ? 'center' : localData.position === 'Gauche' ? '' : 'flex-end' } : {}}>
                <div className={`fr-tile fr-tile--download ${size} fr-enlarge-link mb2`} id="tile-6735"
                style={{marginBottom: marginsBottom[data.espacement_bas]}}>
                    <div className="fr-tile__body">
                        <div className="fr-tile__content">
                            <h3 className="fr-tile__title">
                                <a href={localData.media_a_telecharger.data !== null ?
                                    HOST_URL + localData.media_a_telecharger.data.attributes.url :
                                    localData.telechargement_externe}
                                    download
                                    target="_blank"
                                    data-fr-assess-file="arraybuffer"
                                    hrefLang="fr"
                                    onClick={(e)=> {e.preventDefault(); onDownload(HOST_URL + localData.media_a_telecharger.data.attributes.url, filename)}}
                                    >
                            ...
                            <p className="fr-card__desc">
                                {localData.label &&
                                    <span className={`card-label ${labelColors[localData.label.couleur_du_label]}`}
                                        style={{fontSize:'14px', display:'block', marginTop: '0px',marginBottom:'16px'}}>{localData.label.titre_du_label}
                                    </span>
                                }
            ...
            <div key={localData.id} style={rows == 1 ? { display: 'flex', justifyContent: localData.position === 'Centre' ? 'center' : localData.position === 'Gauche' ? '' : 'flex-end' } : {}}>
                <div className={`fr-card fr-enlarge-link fr-card--horizontal fr-card--horizontal-half ${size} mb2`}
                style={{marginBottom: marginsBottom[data.espacement_bas]}}>
                    <div className="fr-card__body">
                        <div className="fr-card__content">
                            ...
                            <div className="fr-card__end">
                                <svg xmlns="http://www.w3.org/2000/svg" style = {{width:'25px', position:'absolute', right:'24px', bottom: '24px'}} viewBox="0 0 24 24"><path d="M3 19H21V21H3V19ZM13 13.1716L19.0711 7.1005L20.4853 8.51472L12 17L3.51472 8.51472L4.92893 7.1005L11 13.1716V2H13V13.1716Z" fill="#000091"></path></svg>
                            </div>
                        </div>
                    </div>
                    {imageUrl &&
