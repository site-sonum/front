...
export const Button: React.FC<ButtonProps> = ({ data, rows }) => {
  const [buttonData, setButtonData] = useState<ButtonProps["data"]>(data);
  const [fillIcon, setFillIcon] = useState<string>("");

  useEffect(() => {
    setButtonData(data);
    setFillIcon(
      data.couleur === "bleu" ? "rgb(255,255,255,1)" : "rgb(0, 0, 145)",
    );
  }, [data]);
  ...
  return (
    <>
      {rows === 1 && (
        ...
        >
          <Link
            to={buttonData.page_cible}
            target={
              buttonData.page_cible &&
              buttonData.page_cible.includes("https://")
                ? "_blank"
                : undefined
            }
            className={`${
