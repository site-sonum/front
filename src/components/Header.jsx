...
export const Header = ({alerte, liens_navbar}) => {
    ...
    useEffect(()=>{
        if (liens_navbar){
            ...
            if (location.pathname=='/'){
                ...
                const index = liens_navbar.find(e=>e.attributes.titre.split(' ').join('-').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')==location.pathname.slice(1).split('/')[0])
                current_link = index
                if (index!=null){
    ...
    useEffect(()=>{
        ...
        liens_navbar.map(lien_navbar=>{
            if (currentPath.includes(lien_navbar.attributes.lien_navbar[0].page_cible) && lien_navbar.attributes.lien_navbar[0].page_cible!='/'){
    ...
    return (
                        ...
                        <div className="fr-header__brand-top">
                            <div className="fr-header__logo">
                                <a href="/" title="Accueil - ANCT">
                                    <img src={HOST_URL+"/uploads/rp_6c5a005a88.svg"} style={{width:"4.9rem"}}/>
                                </a>
                            </div>
                            <div className="fr-header__operator">
                                <img className="fr-responsive-img" style={{maxWidth:"9.0625rem"}} src= {HOST_URL+headerImage} alt="Logo Société Numérique"/>
                            </div>
                                    ...
                                    {headerLinks && headerLinks.map((headerLink, index)=>{
                                        return(
                                            <li key = {headerLink.id} className="subspace">
                                                <Link to = {headerLink.page_cible} 
                                                    className="headerlink sublink"
                                                    target={headerLink.page_cible.includes('https') ? '_blank' : null}>
                                                    {headerLink.pictogramme_remixicon && 
                                                    <>
                                                        <svg xmlns="http://www.w3.org/2000/svg" style={{width:'22px', position:'absolute', top:'2px'}} viewBox="0 0 24 24"><path d={giveMePath(headerLink.pictogramme_remixicon)} fill="rgb(0,0,145)"></path></svg>
        ...
        <div className="fr-header__menu fr-modal" id="modal-491" aria-labelledby="button-492">
            <div className="fr-container">
                <button className="fr-btn--close fr-btn" style={{backgroundColor:'white'}} aria-controls="modal-491" title="Fermer">
                    Fermer
                </button>
                        ...
                        liens_navbar.map(lien_navbar=>{
                            return lien_navbar.attributes.lien_navbar[0].__component == 'composants.lien' ? (
                                <li className="fr-nav__item" key = {lien_navbar.id}>
                                    <Link to = {getNestedPath(lien_navbar.attributes.lien_navbar[0].page_cible)+lien_navbar.attributes.lien_navbar[0].page_cible}
                                    className="fr-nav__link"
                                    target={lien_navbar.attributes.lien_navbar[0].page_cible && 
                                        lien_navbar.attributes.lien_navbar[0].page_cible.includes('https://') ? '_blank' : null}
                                    aria-current={selectedLink === lien_navbar ? 'true' : null}
                                    onClick={() => {setSelectedLink(lien_navbar), setCurrentCollapse(null)}}
                                    >
                                    {lien_navbar.attributes.lien_navbar[0].titre_du_lien}</Link>
                                </li>
                            ) : ( 
                                <li className="fr-nav__item" key = {lien_navbar.id}>
                                    <button className="fr-nav__btn"
                                        aria-expanded="false" 
                                        aria-controls={lien_navbar.id}
                                        aria-current = {currentCollapse === lien_navbar.id ? "true" : null}
                                        onClick={(e)=> { if (e.button == 0) setSelectedLink(null)} }
                                        >
                                    ...
                                    <ul className="fr-menu__list">
                                        ...
                                        return (
                                            <li key={data.id}>
                                                <Link to = {getNestedPath(data.page_cible)+data.page_cible} className="fr-nav__link"
                                                aria-current={selectedSubLink === data ? 'true' : null}
                                                target={data.page_cible && data.page_cible.includes('https://') ? '_blank' : null}
                                                onClick={(e) => {if (e.button == 0) setCurrentCollapse(lien_navbar.id)}}
                                                onFocus={() => {setSelectedSubLink(data); setCurrentCollapse(lien_navbar.id)}}
                                                onBlur={() => setSelectedSubLink(null)}
                                                >{data.titre_du_lien}</Link>
                                            </li>
                                            )
                                        })}
                                        </ul>
