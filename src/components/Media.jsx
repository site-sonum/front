...
export const Media = ({data}) => {
    ...
    return(
        ...
        <figure role="group" className={`fr-content-media`} style={{margin: data.position == 'Centre' ? '0 auto' : ''}}>
            {videoUrl && <iframe title="Titre de l'iframe" className="fr-responsive-vid fr-ratio-4x3" src={videoUrl} allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe> }
            {!videoUrl && imageUrl && 
            <div>
                ...
                <figcaption className={`fr-content-media__caption ${width}`} style={{marginLeft:'50px'}}>
                    {data.description}
                    <Link to = {data.lien_du_label} 
                        target={data.lien_du_label && data.lien_du_label.includes('https://') ? '_blank' : null}
                        className="fr-link">
                        {data.titre_du_label}
                    </Link>
                </figcaption>
            ...
            <div className={`fr-transcription ${width}`}>
                <button className="fr-transcription__btn" aria-expanded={data.transcription_ouverte ? 'true' : 'false'} aria-controls={`fr-transcription__collapse-transcription-${data.id}`}>Transcription</button>
                    <div className="fr-collapse" id={`fr-transcription__collapse-transcription-${data.id}`}>
                    <dialog id="fr-transcription__modal-transcription-471" className="fr-modal" role="dialog" aria-labelledby={`fr-transcription__modal-transcription-${data.id}-title`}>
                        <div className="fr-container fr-container--fluid fr-container-md">
                            <div className="fr-grid-row fr-grid-row--center">
                                <div className="fr-col-12 fr-col-md-10 fr-col-lg-8">
                                    <div className="fr-modal__body">
                                        <div className="fr-modal__header">
                                            <button className="fr-btn--close fr-btn" aria-controls={`fr-transcription__modal-transcription-${data.id}`} title="Fermer">
                                                Fermer
                                            </button>
                                        </div>
